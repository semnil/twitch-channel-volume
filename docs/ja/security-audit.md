...
| ページの fetch | `window.fetch` を包み、URL に `gql.twitch.tv` を含む応答だけを `clone()` して読む。応答は改変せずページへ返し、拡張からの新規リクエストは発行しない。理由が `TypeError: Failed to fetch` の未処理 rejection は既定動作を止め、それ以外の rejection はそのまま残す | `node test.js` の fetch hook テスト (ページの 1 回の `fetch` に対し元の引数のままの呼び出しが 1 回だけ出ること・対象外 URL の素通し・応答の同一性) と未処理 rejection のテスト (失敗したリクエストの TypeError は止め、別の文言・別の型は残す) |
