...
| リセット対象 | popup が表示中のチャンネル ID と種別を送り、content script が現在値との完全一致を検証する | `node test.js` の `content rejects a measurement reset for a different channel or media kind` (両方の不一致を投入し `channel mismatch` を読む) |
