...
| ページの fetch | `window.fetch` を包み、URL に `gql.twitch.tv` を含む応答だけを `clone()` して読む。応答は改変せずページへ返し、拡張からの新規リクエストは発行しない。未処理 rejection の既定動作を止めるのは、live・VOD・clip のページで wrapper を通したリクエストが失敗した `TypeError: Failed to fetch` の reason だけで、ページ自身の Promise が同じエラーで reject したもの・他のページで出したリクエスト・それ以外の失敗はそのまま残す | `node test.js` の fetch hook テスト (ページの 1 回の `fetch` に対し元の引数のままの呼び出しが 1 回だけ出ること・対象外 URL の素通し・応答の同一性) と未処理 rejection のテスト (live・VOD・clip のページでのリクエストの失敗は止め、ページにはその Promise と reason がそのまま渡る。ページ自身の Promise の同じエラー・予約パスやチャンネルに着く前に出したリクエスト・別の失敗は残す) |
