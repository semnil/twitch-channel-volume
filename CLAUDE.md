...
├── Takes an element only when Web Audio can carry its audio: a `srcObject`, the page's own
...
├── unhandledrejection listener: prevents the default only for the `TypeError: Failed to fetch` that a
│   request made through the fetch hook on a live, VOD or clip page failed with
...
├── Reports the context's state (`audio-context`) when the context is built, whenever it changes,
│   and as the answer to `resume`. A `resume` builds the context where there is none yet
...
│   audio-context / cross-origin); attached decides audioUnavailable / measurementUnavailable from takenElsewhere
...
├── Resume gestures: `click` and `keydown` at the document, in the capture phase, ask the bridge for
│   a `resume` until it reports the context running, and go back on if it ever stops running
...
├── Gain overlay: a span is inserted as the **next sibling** of `.volume-slider__slider-container`. The mute wrapper and the slider container are siblings in a flex row inside the player controls, so the span sits directly to the right of the slider container. Visibility follows the parent `[data-a-target="player-controls"][data-a-visible]` toggle by itself (it is embedded inside the player controls). Shown only while gain ≠ 1.0 and an audio chain exists; the display is fixed to `%` and does not follow displayUnit. A route change rebuilds the player, and the span already made is inserted beside the new slider container, which moves it out of the one that was replaced
...
│   A `clip-owner:<slug>` alias is removed on every write: nothing is kept for a clip for it to point at
...
- **A page fetch nothing handles**: Chrome reports an unhandled rejection from a page request against the `window.fetch` wrapper the request went through, and records `Uncaught (in promise) TypeError: Failed to fetch` in the extension's error list. A request made while `currentContentIdentity()` names a live, VOD or clip page is made through `requestMadeOnContentPage`, and the TypeError of a request Chrome could not make carries the stack of that call. The bridge's `unhandledrejection` listener prevents the default only for a `TypeError` reading `Failed to fetch` whose stack holds an `at requestMadeOnContentPage (chrome-extension://` frame, which reaches the promise the page is handed as well as a promise chained from it or awaited, and it attaches nothing to either. The page's own promise rejected with the same error, a request made on another page (whatever page it fails on), and a request that fails with anything else, an abort among them, raise their unhandled rejection as before, and so does a failure on a page that sets `Error.stackTraceLimit` to 0, whose stacks hold no frame. A `gql.twitch.tv` request carries the bridge's own reaction, which reads its response. `currentContentIdentity()` leaves out the single-segment paths `TWITCH_RESERVED_PATHS` names, through a mirror of that set in `page-bridge.js` (utils.js is not loaded in the MAIN world), and `node test.js` makes a request on each path in the utils.js set
- **Channel ID strategy**: 
...
- **The ad element**: an ad on a VOD plays in a separate `<video>` while the main element stays paused, and that element ignores the main element's volume and sounds at its own. When the main element is paused and another element is sounding, and its media is one Web Audio can carry, a `MediaElementSource` + `GainNode` is inserted on that element too and `baseline * adGainOffset * (main volume / that element's volume)` is applied. An ad element whose media another origin serves is left alone like any other and nothing is reported for it, so the ad gain does not reach it and it keeps playing at its own volume. The gain returns to 1.0 when the ad ends, and the chain is detached once the element leaves the DOM. No measurement is taken from this element
...
- **Media another origin serves**: a `MediaElementAudioSourceNode` outputs silence when its element loaded a cross-origin resource outside CORS mode, and the element's audio does not return to the player once the node exists — so an element like that is left alone rather than taken. Twitch clips are served that way: the clip element plays an MP4 from a CDN and carries no `crossorigin` attribute, while Live and VOD arrive through a `MediaSource` on the page's own origin. An element counts as reachable when it has a `srcObject`, when its resource comes from the page's origin, or when it carries a `crossorigin` attribute; an element that has loaded nothing yet is neither, and the attach loop asks again a second later. An element like that is passed over while another one on the page can be taken, and is only offered to `attach` when it is the last one left — so a page carrying both keeps its gain, and a page carrying only that element still gets the refusal reported. The refusal reaches content.js as `attach-failed` with the cause `cross-origin`, which turns off gain and measurement for that element and states it in the popup. The report is made once per element by the player path alone; the ad path keeps a record of its own so that it cannot spend it. content.js logs that cause at info and every other cause at warn (Chrome collects a content script's warnings as extension errors, and this is the one cause with nothing for the viewer to do about it). Asking for the clip in CORS mode instead is not open to us: with `crossorigin` set, the range requests the player makes come back without an `Access-Control-Allow-Origin` header and the clip does not load at all
- **A clip is not volume-adjusted**: every Twitch clip is served the way above, so the extension has no way to reach a clip's audio, and a clip is out of scope rather than a case that sometimes works. It keeps no channel, no stored gain and no Auto setting, makes no owner lookup, and the popup shows the status line alone instead of a screen of controls that cannot do anything. What an earlier version stored for a clip is removed on the store's next write, down to the row and the provisional name if the clip was all that row held
- **A gesture is made against a state, and applies to that state alone**: every answer content.js gives carries `appliesTo` — the channel, the media kind and the video the URL names, derived from the state rather than counted, so there is no marker to advance. The popup echoes it with the slider, the suggested-gain button, the Auto toggle and the measurement reset, and a gesture whose state has moved on is refused with `state moved` rather than applied to the state that replaced it. Without it `setGain` took the channel from whatever the tab held when the gesture arrived: a slider released during a route change wrote one channel's gain into another channel's entry, under the kind that channel was being watched in (measured). The video is part of the state because two VODs of one owner share a channel id and a kind once the bridge has named the owner, and the measurement the popup is showing belongs to the one that was playing
...
- **Packaging selects by reference graph**: `pack.py` selects what the manifest reaches (the js and css of content_scripts / web_accessible_resources / service_worker / options_page and options_ui.page / action.default_popup / devtools_page / side_panel.default_path / chrome_url_overrides / sandbox.pages / storage.managed_schema / declarative_net_request.rule_resources / icons and action.default_icon, the last written either as one path or as a size for each), plus the `<script src>` and the `<link href>` css of those HTML files, a stylesheet's own `@import` and `url()`, a worker's `importScripts`, and `_locales/<locale>/messages.json` — nothing else. A file nothing references stays out whatever its extension. Paths go through a single resolver, and an absolute path, a `..`, or a symlink (including one on an intermediate directory) whose target leaves the package is made to fail (neither a broken zip nor a zip carrying an outside file is produced silently)
...
MUTATE_CONFIRM="node test.js" sh tools/mutation/sweep-all.sh <worktree> <report> \
  "sh tools/mutation/fast-suite.sh" utils.js content.js page-bridge.js
...
- Documentation is maintained in English and Japanese, English first: `README.md` / `README.ja.md` at the root, `docs/en/` / `docs/ja/` under `docs`. Both members of a pair carry the same headings in the same order. This overrides the global CLAUDE.md default of writing documentation in Japanese
...
- A clip field left by an earlier version (`gainClip`, `autoGainClip`, `autoApplyLoudnessClip`, the `clip` key of `lastLufs` / `lastLufsRef` / `lastLufsWindows` / `autoGainRef`, and their `__fieldVersions` entries) is dropped by `withoutClipFields` the next time channel-store.js writes. `lastMeasuredAt` goes with the last `lastLufs` value on the row. A row this leaves holding no value at all — no gain, no Auto choice, no measurement, no legacy `gain` / `autoApplyLoudness`, only the name, login and URL — was a row for the clip alone and is deleted with it. A row that already held nothing before this ran is not touched by it, and neither is the `clip-owner:<slug>` alias's target
...
- Diagnosing the measurement pipeline: read the `[TCV]` logs in the DevTools Console. They appear in the order `waiting for <video>` → `attached to video` → `measurement chain ready` → `first measurement block received`. `media served from another origin without CORS; the element keeps the player audio path` says the element was passed over (`ad: true` marks the ad path, which reports nothing to content.js, so on that path this line is the only record), and `player audio unavailable` is content.js naming the cause it was given. Stopping at `createMediaElementSource failed` is a conflict with another extension (a technical limit). That state surfaces in the popup notice and in `getState`'s audioUnavailable. Later retries print `no attachable <video>; the player audio is held elsewhere` (`waiting for <video>` appears only when the element itself is absent). `audio context unavailable` / `audio context resume failed` / `audio context stayed <state> after resume` diagnose the same path. Every gesture asks for a resume until the context runs, so the last of those is printed once per stretch the context spends refusing rather than once per gesture
...
- Diagnosing a failed action: a failure is left named, on the screen it happened on. The page side (the Console of the Twitch tab) has `[TCV] failed to save gain` (which names the channel it tried to save, not the one on screen after a navigation) / `[TCV] failed to save Auto setting` / `[TCV] failed to reset measurement`, and, for the save the measurement makes by itself, `[TCV] measurement not persisted` where a stretch of failures starts and `[TCV] failed to persist measurement and Auto gain` once that stretch has lasted five seconds. A block arrives ten times a second and repeats a failed save, so the stretch is named once at each level and a save that gets through ends it; a rejection that reaches a page the extension no longer runs in is named nowhere. Plus the terminal handlers' `[TCV] failed to handle a bridge message` / `[TCV] failed to handle a route change` / `[TCV] failed to start up` / `[TCV] unknown command`. The popup side (the popup's DevTools) has `[TCV] suggested gain request failed` / `[TCV] gain request failed` / `[TCV] Auto setting request failed` / `[TCV] measurement reset request failed`, carrying the reason content.js returned, and `[TCV] state request failed`, carrying the reason `getState` did not arrive at all: no Twitch tab to ask (the manifest asks for twitch.tv alone and for no tabs permission, so Chrome withholds every other tab's URL), or nothing there to answer (a page opened before the extension, a tab that has gone, a runtime the reload invalidated). The first is logged at info and the others at warn: Chrome collects the popup's warnings as extension errors, and a popup opened over another site, or left open while its tab navigates away from Twitch, has nothing for the viewer to put right. The display polls every second, so that one is named once for the stretch the failure lasts and again once the next request arrives. Of the four callers that re-read the state after a failure of their own, the two inside a `catch` ask for no status line and keep the message they put up until the next poll takes the whole screen; `applyMeasured` and `setGain` re-read with the status line on, and on a page that cannot answer at all their own message is never drawn (it is written by `renderState`, which returns on the state it did not get). The options side (the settings page's DevTools) has `[TCV] failed to delete the channel` / `[TCV] failed to clear the saved channels`. The Service Worker side (the extension's Service Worker DevTools) has `[TCV] channelVolumes mutation failed` / `[TCV] settings mutation failed` (from storage), `[TCV] channelVolumes mutation rejected as invalid` / `[TCV] settings mutation rejected as invalid` (the caller's input), `[TCV] channelVolumes mutation blocked by the stored state` / `[TCV] settings mutation blocked by the stored state` (the stored state), and `[TCV] unknown message type`
