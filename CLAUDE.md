...
- **The ad element**: an ad on a VOD plays in a separate `<video>` while the main element stays paused, and that element ignores the main element's volume and sounds at its own. When the main element is paused and another element is sounding, and its media is one Web Audio can carry, a `MediaElementSource` + `GainNode` is inserted on that element too. An ad element whose media another origin serves is left alone like any other, and nothing is reported for it — the ad gain does not reach it, and the element keeps playing at its own volume and `baseline * adGainOffset * (main volume / that element's volume)` is applied. The gain returns to 1.0 when the ad ends, and the chain is detached once the element leaves the DOM. No measurement is taken from this element
