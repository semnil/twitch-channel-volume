...
| `gen_screenshots.py` | スクリーンショット生成 (PIL 直接描画, 640×400 ja/en → `docs/screenshots/`)。`--check` は書き込まず画素比較 (差は exit 1、描けない環境は exit 3)、`--out <dir>` は書き込み先を差し替える |
