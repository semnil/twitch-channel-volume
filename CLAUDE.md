...
- **A page fetch nothing handles**: Chrome reports an unhandled rejection from a page request against the `window.fetch` wrapper the request went through, and records `Uncaught (in promise) TypeError: Failed to fetch` in the extension's error list. A request made while `currentContentIdentity()` names a live, VOD or clip page gets a rejection handler that records its reason in a `WeakSet` when the reason is a `TypeError` reading `Failed to fetch`, and the bridge's `unhandledrejection` listener prevents the default for a reason in that set alone. The page's own promise rejected with the same error, a request made on another page (whatever page it fails on), and a request that fails with anything else are left as they are. The handler holds the request's promise as handled, so a request made on such a page that the page leaves with no handler at all raises no unhandled rejection, whatever it fails with. `currentContentIdentity()` leaves out the single-segment paths `TWITCH_RESERVED_PATHS` names, through a mirror of that set in `page-bridge.js` (utils.js is not loaded in the MAIN world), and `node test.js` makes a request on each path in the utils.js set
- **Channel ID strategy**: 
