...
| `gen_screenshots.py` | スクリーンショット生成 (PIL 直接描画, 640×400 ja/en → `docs/screenshots/`)。`--check` は書き込まず RGBA のバイト列で比較し (大きさとフレーム数は先に別途)、差は exit 1、描けない環境は exit 3。`--out <dir>` は書き込み先を差し替える。知らない引数は exit 2 で拒否する (`--chek` が上書きにならないように) |
